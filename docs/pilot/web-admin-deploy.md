...
2. `npm run check:admin-artifact` — fails unless that freshly built output is unmistakably the
   **admin** app (admin route documents present, consumer-only documents absent, `_headers`
   shipped);
3. `npm run check:admin-routing` — starts a local Wrangler assets server on an isolated port
   and proves `/`, `/login`, `/bookings` and `/bookings/<id>` all return 200, that the
   dynamic route is served **through** `index.html`, and that no consumer-only document leaked in.
   It shuts the server down in a `finally` path and verifies the port was released.
4. `wrangler deploy -c apps/admin/wrangler.jsonc` — the explicit, non-discovering deploy.
...
npm run check:admin-routing  # real Wrangler routing smoke on its own
```
...
### `index.html` is a MANDATORY build output

The fallback above can only work if the export actually produced `dist/index.html`. It is
generated by the root route `apps/admin/src/app/index.tsx` — delete that route and the file
disappears silently.

That is not hypothetical. A build shipped without it and Cloudflare returned **404** for:

- the bare root `/`; and
- **every dynamic route on a hard refresh**, such as `/bookings/<id>`.

`/bookings` kept working (it has its own `bookings/index.html`) and client-side navigation kept
working (it never requests a document), so the failure looked intermittent rather than structural.

**Neither of these is a substitute:**

- `+not-found.html` — an Expo route reachable only after hydration. Cloudflare`s SPA mode
  serves `/index.html` specifically; it never consults this file.
- `bookings/[id].html` — a literal filename containing brackets. No static host can map
  `/bookings/<id>` onto it.

Two guards enforce this: `npm run check:admin-artifact` fails if `index.html` is absent, and
`npm run check:admin-routing` starts a real local Wrangler server and proves the fallback resolves.
...
- [ ] The **bare root** `/` returns 200 (not 404) and lands on login or the dashboard.
- [ ] A deep link such as `/bookings/<id>` returns 200 and renders after a **hard refresh**.
      This is the case that regressed in production; a client-side click is NOT sufficient
      evidence, because it never requests a document.
