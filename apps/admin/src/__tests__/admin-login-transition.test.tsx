...
let mockSegments: string[] = ['index', 'login'];
...
const mockReplaceSpy = jest.fn((href: string) => {
  // Phase 3G: admin login redirects to /dashboard (the dashboard moved off "/").
  if (href === '/dashboard') mockSetSegments(['index', 'dashboard']);
  else if (href.includes('login')) mockSetSegments(['index', 'login']);
  ...
  return {
    ...
    Slot: () => {
      ...
      const Screen = onLogin
        ...
        ? require('@admin/app/login').default
        : require('@admin/app/dashboard').default;
...
test('admin login → Loading (protected UI hidden) → dashboard, no error boundary, one navigation', async () => {
  ...
  expect(mockReplaceSpy.mock.calls.filter((c) => c[0] === '/dashboard').length).toBe(1);
