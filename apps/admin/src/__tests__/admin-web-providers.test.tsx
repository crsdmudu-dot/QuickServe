 ...
 *   - src/app/providers/index.tsx  (list)
 *   - src/app/providers/[id].tsx   (detail)
 *
...
import AdminWebProvidersScreen from '@admin/app/providers/index';
...
describe('AdminWebProvidersScreen (list)', () => {
  ...
  it('navigates to provider detail when a row is pressed', async () => {
    ...
    fireEvent.press(row);
    await waitFor(() =>
      expect(router.push).toHaveBeenCalledWith('/providers/prov1'),
    );
  });
});
