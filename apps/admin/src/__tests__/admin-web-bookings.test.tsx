 ...
 *   - src/app/bookings/index.tsx  (list)
 *   - src/app/bookings/[id].tsx   (detail)
 *
...
// quotes: the admin-only functions moved to the admin application's own module when they
// were split out of the shared @/lib/quotes; mock them where they now live.
jest.mock('@admin/lib/quotes-admin', () => ({
  setBookingQuote: jest.fn().mockResolvedValue({ ok: true }),
}));
...
// wallet: the admin-only functions moved to the admin application's own module when they
// were split out of the shared @/lib/wallet; mock them where they now live.
jest.mock('@admin/lib/wallet-admin', () => ({
  adminGetWallet: jest.fn().mockResolvedValue({
    id: 'w1',
    customer_id: 'cust1',
    balance: 0,
...
import AdminWebBookingsScreen from '@admin/app/bookings/index';
import AdminWebBookingDetailScreen from '@admin/app/bookings/[id]';
...
describe('AdminWebBookingsScreen (list)', () => {
  ...
  it('navigates to the booking detail when a row is pressed', async () => {
    ...
    fireEvent.press(row);
    await waitFor(() =>
      expect(router.push).toHaveBeenCalledWith('/bookings/bk1'),
    );
  });
...
describe('AdminWebBookingDetailScreen (detail)', () => {
  ...
  // Service-label 3-step fallback on the admin surface. These two cases came from the native
  // /admin/booking/[id] suite, which was retired when administration was separated out of the
  // consumer app; the assertions are preserved here against the admin web booking detail.
  it('unknown service slug → humanised fallback renders, no crash', async () => {
    mockGetBookingById.mockResolvedValueOnce({ ...MOCK_BOOKING, service_id: 'brand-new-unknown-service' });
  ...
  // ── Service Details V1.4 — admin surface 2 (admin web) ────────────────────
  ...
  it('renders a legacy booking (no snapshot) safely and leaves dispatch actions unchanged', async () => {
    // MOCK_BOOKING predates Service Details.
    render(<AdminWebBookingDetailScreen />);
