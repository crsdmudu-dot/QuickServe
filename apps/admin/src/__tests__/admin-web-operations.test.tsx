 ...
 *   - src/app/operations/index.tsx  (list)
 *   - src/app/operations/new.tsx    (create)
 *   - src/app/operations/[id].tsx   (detail)
...
import AdminWebOperationsScreen from '@admin/app/operations/index';
import AdminWebOperationsNewScreen from '@admin/app/operations/new';
import AdminWebOperationDetailScreen from '@admin/app/operations/[id]';
...
describe('AdminWebOperationsScreen (list)', () => {
  ...
  it('"New case" button navigates to /operations/new', () => {
    render(<AdminWebOperationsScreen />);
    fireEvent.press(screen.getByText('New case'));
    expect(router.push).toHaveBeenCalledWith('/operations/new');
  });
  ...
  it('row press navigates to case detail', async () => {
    ...
    fireEvent.press(row);
    await waitFor(() =>
      expect(router.push).toHaveBeenCalledWith('/operations/case-1'),
    );
  });
});
