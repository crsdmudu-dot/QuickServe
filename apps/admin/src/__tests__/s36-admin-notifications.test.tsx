...
// notifications: the admin-only functions moved to the admin application's own module when they
// were split out of the shared @/lib/notifications; mock them where they now live.
jest.mock('@admin/lib/notifications-admin', () => ({
  broadcastAnnouncement: (...a: any[]) => mockBroadcastAnnouncement(...a),
}));
...
import AdminWebNotificationsScreen from '@admin/app/notifications/index';
import AdminBroadcastScreen from '@admin/app/broadcast';
...
describe('AdminWebNotificationsScreen (enhanced Slice 36 Task 5)', () => {
  ...
  it('pressing Broadcast button navigates to broadcast route', async () => {
    ...
    expect(router.push).toHaveBeenCalledWith('/broadcast');
  });
...
describe('resolveNotificationDeepLink — all supported types', () => {
  ...
  it('resolves new_support_case with metadata id → /operations/:id', () => {
    ...
    expect(result).toBe('/operations/case-123');
  });

  it('resolves new_dispute with metadata id → /operations/:id', () => {
    mockResolveNotificationDeepLink.mockImplementationOnce((n: any) => {
      const id = n.metadata_json?.id;
      if (id) return `/operations/${id}`;
      return null;
    ...
    expect(result).toBe('/operations/dispute-456');
  });
...
describe('Admin NotificationBell', () => {
  ...
  it('navigates to admin notifications when wired via router.push', () => {
    const handlePress = () => router.push('/notifications' as Href);
